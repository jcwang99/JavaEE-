	...
	<script type="text/javascript">
			...
			function login(){
				if('WebSocket' in window){
					...
					websocket = new WebSocket("ws://localhost:8080/work5/chatDemo/" + sendUser);
					//alert('Success')
		        }else{
			...
			//连接发生错误的回调方法
	          ...
	          websocket.onmessage = function(event){
	        	  ...
	        	  }else if(event.data.indexOf("online")>-1){
	        		  var msg = event.data;
	        		  var data = msg.split(":");
	        		  document.getElementById('online').innerHTML=data[1];
	        	  }else{
...
<body>
            <%String sendUser=String.valueOf(session.getAttribute("login"));%>
			操　作:<input type="hidden" name="sendUser" id="sendUser" value=<%= sendUser %>/> 
